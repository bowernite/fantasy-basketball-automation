...
- **`GPp` is `sim.project_gp`:** mean of Hashtag and FanScout projected GP when both hit; the one feed if only one hits; the durability map only if neither (`projections`). Map is fallback, not a vote. Refresh both feeds via `hashtag_gp.py refresh` / `fanscout_gp.py refresh`.
