...
**`GP` is `sim.durable_gp`:** `GPp`, floored at the durability map (`Durability.md`). A `GPp` under the map is a this-season absence (injury, suspension, override) — `Δw` ignores it; `Δw (season)`, `ΔP(title)` and `W20`–`W23` price it. A body the pool has never seen (rookie, pick) keeps `GPp`.
