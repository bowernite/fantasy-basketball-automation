...
import fetch_data
import sim
from simlib import bracket, engine, gp, roster as roster_mod, shard, value
...
class ShardedReports(unittest.TestCase):
    ...
    feeding the next -- a break-even bisects, so a last-digit disagreement moves
    the bracket it searches and prints a different rate. Nothing else here runs
    a report sharded at all: every other report test goes through
