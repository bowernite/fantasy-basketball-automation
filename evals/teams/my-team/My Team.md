...
§*`TVAL`* below) — printed adjacent to BASE, the pair BASE/`TVAL` are the two decision
columns, and the gap between them is the buy/sell signal. All are separate layers; never
blend them.

## `TVAL` — added 2026-07-30

`TVAL = κ × Σₜ δ^(t−t₀) · Mₜ · Δwₜ`, BASE units, printed adjacent to BASE below. `κ` = **940**
(sim layer, §`kappa`), `Σδ` = **2.641** (δ=0.875, 3-season horizon, §header). `Mₜ` = **1.0
flat across all 3 seasons** — `team-projections.md` has us projected **2nd / 2nd / 4th**, all
top-4 finishes, which `team-eval`'s table reads as `M ≈ 1.0` ("top seed") in every one of
those seasons; no bubble or out-of-contention year falls inside this horizon. Same M used in
`../matthew/matthew.md`'s `TVAL` derivation for consistency (`../matthew/matthew-trade-strategy.md`
instead uses a conservative `M=0.9` for one specific deal's verdict inequality — a judgment
call for that negotiation, not restated here). `Δwₜ` is held **flat** at the already-published
sim-measured `Δw` column for the horizon (`team-eval`: no aging term inside `Δw`, no per-player
truncation applied here beyond what `LATE`/AGE already flag qualitatively).

So per player: `TVAL = 940 × 2.641 × 1.0 × Δw = 2482.5 × Δw`. **A positive linear rescale of
`Δw`, so the existing Δw-sort is already the TVAL-sort** — no reordering was needed. `TVAL`
carries `κ`'s band (630-1312 quartiles): rescale the band the same way, `2482.5 × Δw` sits at
the point estimate, `(630 × 2.641) × Δw` to `(1312 × 2.641) × Δw` = `1664 × Δw` to `3465 × Δw`
brackets it. Never present the band as a second sourced figure — it's modelled, same as the
point estimate.
...
valuable. See `../mitch/mitch.md` / `../matthew/matthew.md`, which publish both columns.
...
Sorted by **`TVAL`** (`team-eval` §Output: "sort on TVAL, ours"). `TVAL` is a positive linear
rescale of the sim's **`Delta w`** measurement (§*`TVAL`* above), so this order also matches
`sim.py players`'s `Delta w` order exactly — no rows moved when the column was added.
...
BASE ~0. They carry real ranks. `../../board-snapshots/boards-2026-07-29.md` has the worked cases.

| Player | dizP | htP | crd | **BASE** | **TVAL** | FPts/G | GP | GP proj | **WINS** | **Δw** | σ | **LATE** | AGE | ELIG |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---:|---|
| Kawhi Leonard | 68 | 107 | 90 | **1724** | **5213** | 44.4 | 65 | 63 | 2.29 | +2.10 | 17.1 | ✗ age 35 · 2/4 under 60 | 35.1 | SF/PF |
| Cade Cunningham | 6 | 6 | 5 | **8163** | **4791** | 46.7 | 65 | 62 | 2.28 | +1.93 | 3.2 | ✓ 1 of 5 under 60 | 24.8 | PG/SG |
| Amen Thompson | 18 | 24 | 14 | **5360** | **4742** | 39.3 | 79 | 68 | 2.03 | +1.91 | 46.9 | ✓✓ none under 60 in 3 | 23.5 | SG/SF |
| Josh Giddey | 24 | 28 | 44 | **4137** | **3699** | 42.2 | 54 | 59 | 1.83 | +1.49 | 6.4 | ✓ 2 of 5 under 60 | 23.8 | PG/SG |
| Jalen Duren | 35 | 36 | 27 | **3817** | **3525** | 39.2 | 70 | 64 | 1.75 | +1.42 | 15.7 | ✓✓ none under 60 in 4 | 22.7 | C |
| Jimmy Butler | 181 | 153 | 154 | **777** | **3128** | 37.6 | 38 | 53 | 1.46 | +1.26 | 9.3 | ✗ age 37 · 3/5 under 60 | 36.9 | SF/PF |
| Kyrie Irving | 70 | 95 | 122 | **1650** | **2855** | *38.0* | 0 | 57 | 1.46 | +1.15 | 21.2 | ✗ age 34 · 3/4 under 60 | 34.4 | PG/SG |
| Desmond Bane | 57 | 66 | 67 | **2309** | **2309** | 33.0 | 82 | 69 | 1.32 | +0.93 | 12.2 | ✓ 2 of 5 under 60 | 28.1 | PG/SG |
| Naz Reid | 105 | 77 | 104 | **1553** | **2036** | 27.4 | 77 | 66 | 0.96 | +0.82 | 1.1 | ✓✓ none under 60 in 5 | 26.9 | PF/C |
| Darius Garland | 32 | 43 | 58 | **3281** | **1986** | 31.9 | 45 | 55 | 0.98 | +0.80 | 8.8 | ✓ 2 of 5 under 60 | 26.5 | PG/SG |
| Zach Edey | 51 | 61 | 69 | **2451** | **1539** | 33.7 | 11 | 43 | 0.88 | +0.62 | 0.8 | – 2 rotation seasons (66/11) | 24.2 | C |
| Fred VanVleet | 134 | 74 | 161 | **1295** | **1514** | *29.0* | 0 | 60 | 0.84 | +0.61 | 4.4 | ✗ missed '25-26 whole | 32.4 | PG/SG |
| Jalen Suggs | 80 | 92 | 103 | **1630** | **1415** | 30.5 | 57 | 60 | 0.96 | +0.57 | 11.5 | ✗ 4 of 5 under 60 | 25.2 | PG/SG |
| Myles Turner | 129 | 104 | 135 | **1162** | **1167** | 24.7 | 71 | 63 | 0.70 | +0.47 | 9.9 | ✓ 1 of 5 under 60 | 30.3 | PF/C |
| Jakob Poeltl | 133 | 135 | 307 | **820** | **869** | 25.8 | 46 | 54 | 0.55 | +0.35 | 5.9 | ✗ 3 of 5 under 60 | 30.8 | C |
| **Marvin Bagley** → released | 317 | 453 | 637 | **82** | **670** | 22.8 | 60 | 58 | 0.50 | +0.27 | 1.6 | ✗ 3 of 4 under 60 | 27.4 | SF/PF |
| Coby White | 93 | 113 | 106 | **1400** | **621** | 25.8 | 50 | 55 | 0.55 | +0.25 | 3.7 | ✓ 1 of 5 under 60 | 26.5 | PG/SG |
| Jay Huff | 271 | 223 | 534 | **288** | **521** | 20.7 | 82 | 65 | 0.24 | +0.21 | 5.7 | – 2 rotation seasons (7/82) | 28.9 | C |
| De'Anthony Melton | 229 | 231 | 418 | **342** | **422** | 22.7 | 49 | 54 | 0.32 | +0.17 | 3.4 | ✗ 3 of 5 under 60 | 28.2 | PG/SG |
| Anfernee Simons | 198 | 155 | 155 | **734** | **372** | 21.4 | 55 | 55 | 0.24 | +0.15 | 17.2 | ✗ 3 of 5 under 60 | 27.1 | PG/SG |
| Khris Middleton | 347 | 331 | 553 | **119** | **99** | 18.8 | 63 | 57 | 0.20 | +0.04 | 25.2 | ✗ age 35 · 3/5 under 60 | 35.0 | SF/PF |
| Adem Bona | 284 | 303 | 410 | **205** | **-223** | 14.2 | 71 | 58 | -0.14 | -0.09 | 0.9 | – 0 rotation seasons | 23.3 | SF/PF |
| Chaney Johnson | - | - | - | **0** | **-248** | 19.1 | 18 | 40 | 0.15 | -0.10 | 2.6 | – 1 rotation season (17) | 24.1 | SG/SF |
| Karlo Matković | 338 | 342 | 448 | **121** | **-323** | 14.2 | 62 | 55 | -0.13 | -0.13 | 0.9 | – 1 rotation season (42) | 25.3 | PF/C |
| Khaman Maluach | - | 154 | 340 | **559** | **-348** | *16.0* | 46 | 50 | -0.12 | -0.14 | 4.1 | – 0 rotation seasons | 19.9 | C |
| Sion James | 384 | 294 | 344 | **161** | **-472** | 13.9 | 82 | 62 | -0.33 | -0.19 | 0.8 | – 0 rotation seasons | 23.7 | PG/SG |
| Keon Ellis | 346 | 214 | 411 | **248** | **-497** | 14.7 | 72 | 59 | -0.25 | -0.20 | 0.4 | – 1 rotation season (80) | 26.6 | PG/SG |
| DaRon Holmes | 310 | 333 | 390 | **166** | **-521** | *14.0* | 26 | 41 | -0.11 | -0.21 | — | – 0 rotation seasons | 24.0 | SF/PF |

**BASE↔`TVAL` gap, where it isn't inside `κ`'s band (630-1312 quartiles, i.e. `TVAL` point
estimate rescales to a **1664×Δw – 3465×Δw** range per row):** Cade, Amen, Giddey, Duren
(BASE far above TVAL — the board's youth premium, not a sell signal per `team-eval`), and
Kawhi, Butler, Kyrie, Bane, Reid, Coby, Melton (TVAL above or near BASE — durable, productive
vets the boards discount for age that our win-now window pays for). Turner sits almost exactly
on BASE (1162 vs 1167) — no gap, no signal. Maluach's gap is the sharpest inversion in the
file: BASE **559** (2nd-highest bottom-of-roster piece) against `TVAL` **-348** — a 19-year-old
the board likes and the sim currently does not, which is a `WINS`-layer read (rate/GP, not
age) and not a reason to change how `trade-targets.md` prices shipping him.
